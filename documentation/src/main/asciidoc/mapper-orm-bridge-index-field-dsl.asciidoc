...
The field, then, is considered _dynamic_.

Dynamic fields are not declared on bootstrap,
but need to match a field _template_ that is declared on bootstrap.
The template includes the field type and structural information (multi-valued or not, ...),
but omits the field name.

Field templates are declared with the `fieldTemplate` methods, as shown below.
