...
Selects the index <<elasticsearch-index-schema-management-strategy,creation strategy>>:: `hibernate.search.default.elasticsearch.index_schema_management_strategy CREATE`
...
===== [[elasticsearch-index-schema-management-strategy]] Index schema management strategies
...
[NOTE]
--
Mapping validation is as permissive as possible. Fields or mappings that are unknown to Hibernate Search will be ignored, and settings that are more powerful than required (i.e. a field annotated with `@Field(store = Store.NO)` in Search but marked as `"store": true` in Elasticsearch) will be deemed valid.
